...
export function MessageThread({ messages, mySlot, onSend, isSending, accentColor, onTyping, isOtherTyping, draftKey }: MessageThreadProps) {
  ...
  function handleInput(e: React.ChangeEvent<HTMLTextAreaElement>) {
    const ta = e.target; const value = ta.value; setDraft(value)
    try {
    ...
    const now = Date.now()
    if (now - lastTypingRef.current >= THROTTLE_MS) { lastTypingRef.current=now; onTyping() }
  }

  async function handleSend() {
    const trimmed = draft.trim()
    if (!trimmed || isSending) return
    setDraft(''); if (textareaRef.current) textareaRef.current.style.height='auto'
    try { sessionStorage.removeItem(draftKey) } catch {}
    await onSend(trimmed); textareaRef.current?.focus()
  }
    ...
    <>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:0, fontFamily:"'Figtree',system-ui,sans-serif" }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10, flexShrink:0 }}>
          ...
          {messages.length > 0 && <span style={{ marginLeft:'auto', color:'var(--th-text-4)', fontSize:'0.60rem', fontWeight:500 }}>{messages.length}</span>}
        </div>

        <div ref={scrollRef} className="mt-scroll" style={{ overflowY:'auto', maxHeight:192, minHeight:56, display:'flex', flexDirection:'column', gap:8, paddingRight:4, marginBottom:10 }}>
          {messages.length === 0 && !isOtherTyping ? (
            ...
            <>
              {messages.map(msg => {
                ...
                return (
                  <div key={msg.id} className="mt-msg" style={{ display:'flex', flexDirection:'column', alignItems:isMe?'flex-end':'flex-start', gap:3 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:5, flexDirection:isMe?'row-reverse':'row' }}>
                      <span style={{ fontFamily:"'Syne',system-ui,sans-serif", color:isMe?`${accentColor}99`:'var(--th-text-3)', fontSize:'0.58rem', fontWeight:700, letterSpacing:'0.06em' }}>{msg.player_name}</span>
                      <span style={{ color:'var(--th-text-4)', fontSize:'0.55rem', fontFamily:"'Figtree',system-ui,sans-serif" }}>{formatTime(msg.created_at)}</span>
                    </div>
                    <div style={{ maxWidth:'84%', padding:'8px 12px', borderRadius:isMe?'14px 14px 4px 14px':'14px 14px 14px 4px', background:isMe?`${accentColor}14`:'var(--th-surface)', border:isMe?`1px solid ${accentColor}2a`:'1px solid var(--th-border)', color:isMe?'var(--th-fg)':'var(--th-text-2)', fontSize:'0.82rem', fontWeight:400, lineHeight:1.52, wordBreak:'break-word', fontFamily:"'Figtree',system-ui,sans-serif" }}>
                      {msg.content}
                    </div>
                  </div>
                )
        ...
        <div style={{ display:'flex', gap:8, alignItems:'flex-end', flexShrink:0 }}>
          <textarea ref={textareaRef} value={draft} onChange={handleInput} onKeyDown={handleKeyDown}
            placeholder="Type your response..." maxLength={500} rows={1}
          ...
          <button onClick={handleSend} disabled={!canSend} aria-label="Send message"
            ...
            {isSending ? <div className="mt-send-spinner" style={{ color:accentColor }}/> : (
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M14.5 1.5L1 6.5l5 2 2 5 1.5-4.5L14.5 1.5z" fill="currentColor" fillOpacity="0.88" strokeLinejoin="round"/><line x1="6" y1="8.5" x2="9.5" y2="5" stroke="currentColor" strokeWidth="0.8" strokeOpacity="0.5" strokeLinecap="round"/></svg>
            )}
          </button>
        </div>
