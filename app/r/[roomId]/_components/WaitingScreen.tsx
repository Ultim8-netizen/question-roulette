    ...
    <>
      <style>{`
        ...
        .ws-confirm-btn {
          ...
          color:#fbbf24; font-family:'DM Sans',system-ui,sans-serif;
      ...
      <div className="ws-root" style={{
        ...
        <div className="ws-mark-anim" style={{ marginBottom:16 }}>
          <Room13Mark size={36} opacity={1} />
        </div>

        <span style={{ fontFamily:"'Geist Mono',ui-monospace,monospace", fontSize:'0.70rem', fontWeight:500, letterSpacing:'0.22em', color:'var(--th-text-4)', textTransform:'lowercase', marginBottom:28 }}>
          room 13
        </span>
        ...
        <h1 style={{ fontFamily:"'Cormorant Garamond',serif", color:'var(--th-text-1)', fontSize:'1.55rem', fontWeight:600, textAlign:'center', marginBottom:8 }}>
          Waiting for them
        </h1>

        <p style={{ color:'var(--th-text-3)', fontSize:'0.82rem', textAlign:'center', marginBottom:28, lineHeight:1.6 }}>
          Share the link below.<br />The game starts once they join.
        </p>

        {/* Share link copy button */}
        <button className="ws-copy-btn" onClick={copy}>
          <span style={{ flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', color:'var(--th-text-3)', fontSize:'0.72rem', textAlign:'left' }}>
            {shareUrl}
          </span>
          <span style={{ color: copied ? '#4ade80' : 'var(--th-text-2)', fontSize:'0.70rem', fontWeight:600, letterSpacing:'0.08em', flexShrink:0, transition:'color 0.2s ease' }}>
            {copied ? '✓ Copied' : 'Copy'}
          </span>
        </button>

        {/* ── Link safety warning ── */}
        <div className="ws-warn" style={{
          marginTop:20,
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
            ...
            <span style={{ color:'#fbbf24', fontSize:'0.68rem', fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase' }}>
              Save your link — don&apos;t lose your session
            </span>
          </div>

          {/* Explanation */}
          <p style={{ color:'var(--th-text-2)', fontSize:'0.78rem', lineHeight:1.65, margin:'0 0 14px 0' }}>
            This game lives at <strong style={{ color:'var(--th-text-1)', fontWeight:600 }}>your unique link</strong>. If you close this tab without saving it, you may not be able to get back in.
          </p>

          <ul style={{ margin:'0 0 16px 0', padding:'0 0 0 2px', listStyle:'none', display:'flex', flexDirection:'column', gap:7 }}>
            {[
              { icon:'📋', text:'Copy the link above and paste it somewhere safe — notes, messages, anywhere.' },
              { icon:'🔖', text:'Or bookmark this page in your browser right now.' },
              { icon:'🔗', text:'Your partner needs the same link to join — share it with them.' },
            ].map(({ icon, text }) => (
              <li key={icon} style={{ display:'flex', alignItems:'flex-start', gap:9 }}>
                ...
                <span style={{ color:'var(--th-text-3)', fontSize:'0.74rem', lineHeight:1.6 }}>{text}</span>
              </li>
            ))}
          </ul>
          ...
          {/* Confirm checkbox */}
          <button
            className={`ws-confirm-btn${linkSaved ? ' done' : ''}`}
            ...
            {linkSaved ? (
              <>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  ...
                  <path d="M4.5 7l1.8 1.8L9.5 5" stroke="#4ade80" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                Got it — link is saved
              </>
            ) : (
              <>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <rect x="1" y="1" width="12" height="12" rx="3" stroke="#fbbf24" strokeWidth="1.2"/>
                </svg>
                I&apos;ve saved my link
              </>
            )}
