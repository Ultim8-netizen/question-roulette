...
export function ConsentBanner({ proposal, onAccept, onDecline, loading }: ConsentBannerProps) {
  ...
  return (
    <>
      ...
      `}</style>
      <div className="cb-root" style={{ margin:'12px 16px 0', borderRadius:18, overflow:'hidden', background:'var(--th-surface)', border:`1px solid ${color}22`, boxShadow:`0 4px 24px ${color}0e`, padding:'16px 18px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
          <div style={{ width:7, height:7, borderRadius:'50%', background:color, boxShadow:`0 0 8px ${color}` }}/>
          <span style={{ color:'var(--th-text-3)', fontSize:'0.68rem', fontWeight:600, letterSpacing:'0.12em', textTransform:'uppercase' }}>Question proposal</span>
          <span style={{ marginLeft:'auto', color, fontSize:'0.68rem', fontWeight:600, letterSpacing:'0.06em' }}>{TIER_LABELS[proposal.tier]}</span>
        </div>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={onDecline} disabled={loading} style={{ flex:1, height:40, borderRadius:11, background:'transparent', border:'1px solid var(--th-border)', color:'var(--th-text-3)', fontSize:'0.78rem', fontWeight:500, cursor:'pointer', fontFamily:"'DM Sans',system-ui,sans-serif" }}>Decline</button>
          <button onClick={onAccept} disabled={loading} style={{ flex:2, height:40, borderRadius:11, background:`linear-gradient(135deg,${color}18,${color}0c)`, border:`1px solid ${color}33`, color, fontSize:'0.78rem', fontWeight:600, cursor:'pointer', fontFamily:"'DM Sans',system-ui,sans-serif" }}>{loading ? 'Adding...' : 'Accept'}</button>
        </div>
      </div>
    </>
  )
...
// ---------------------------------------------------------------------------
// PlayerHeader — now accepts a themeToggle slot
// ---------------------------------------------------------------------------
...
type PlayerHeaderProps = {
  ...
  /** Rendered at the far right of the header row, in natural document flow. */
  themeToggle?:  React.ReactNode
}

export function PlayerHeader({ p1Name, p2Name, mySlot, myPersonalUrl, themeToggle }: PlayerHeaderProps) {
  return (
    <>
      <style>{`
        .guide-btn {
      ...
      {/*
        Layout: [P1 chip] [VS · guide-btn] [P2 chip] [ThemeToggle]
        ThemeToggle sits at the trailing edge of this row — in flow,
        never overlapping content on first paint.
      */}
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          <span style={{ color:'var(--th-text-4)', fontSize:'0.65rem', fontWeight:600, letterSpacing:'0.12em' }}>VS</span>
        ...
        {/* ThemeToggle: in-flow, trailing edge. No fixed/absolute positioning. */}
        {themeToggle && (
...
// ---------------------------------------------------------------------------
// PlayerChip — copy button now labelled and explained
// ---------------------------------------------------------------------------

function PlayerChip({
  name, isMe, align, myPersonalUrl,
}: {
  name:          string
  isMe:          boolean
  align:         'left' | 'right'
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: isMe ? 'var(--th-text-1)' : 'var(--th-text-3)', fontSize:'0.80rem', fontWeight:400, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
            {name}
            {isMe && <span style={{ color:'var(--th-text-4)', fontSize:'0.60rem', marginLeft:5 }}>you</span>}
          </span>

          {/*
            Copy button — only on your own chip.
            Labelled clearly so new users understand exactly what is copied.
            Tooltip on hover adds further context.
          */}
          {isMe && myPersonalUrl && (
            <div style={{ position:'relative', flexShrink:0 }}>
              <button
                ...
                aria-label={copied ? 'Link copied!' : 'Copy your personal return link'}
              >
                {copied ? (
                  <>
                    <span className="chip-copy-confirm" style={{ fontSize:'0.60rem', lineHeight:1 }}>✓</span>
                    <span className="chip-copy-confirm">Copied!</span>
                  </>
                ) : (
                  <>
              ...
              {/* Tooltip — explains what the link does */}
              {showTip && !copied && (
                <div className="chip-tip" style={{
                  ...
                  <div style={{ color:'var(--th-text-1)', fontSize:'0.68rem', fontWeight:600, marginBottom:4, fontFamily:"'DM Sans',system-ui,sans-serif" }}>
                    Your personal return link
                  </div>
                  <div style={{ color:'var(--th-text-3)', fontSize:'0.62rem', lineHeight:1.55, fontFamily:"'DM Sans',system-ui,sans-serif" }}>
                    Save this to come back to your session from any device or browser — even if you close this tab.
                  </div>
                </div>
              )}
            </div>
